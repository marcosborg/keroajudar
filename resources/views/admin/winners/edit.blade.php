...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.winners.update", [$winner->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('entry') ? 'is-invalid' : '' }}" name="entry_id" id="entry_id" required>
                    @foreach($entries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('entry_id') ? old('entry_id') : $winner->entry->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('entry'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prize') ? 'is-invalid' : '' }}" name="prize_id" id="prize_id" required>
                    @foreach($prizes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prize_id') ? old('prize_id') : $winner->prize->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('prize'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('draw_date') ? 'is-invalid' : '' }}" type="text" name="draw_date" id="draw_date" value="{{ old('draw_date', $winner->draw_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{{ old('notes', $winner->notes) }}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
