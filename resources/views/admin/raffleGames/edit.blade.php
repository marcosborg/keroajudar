...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.raffleGame.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.raffle-games.update", [$raffleGame->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.raffleGame.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $raffleGame->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('prize_id') ? 'is-invalid' : '' }}" name="prize_id" id="prize_id">
                    @foreach($prizes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prize_id') ? old('prize_id') : $raffleGame->prize_id) == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('starts_at') ? 'is-invalid' : '' }}" type="datetime-local" name="starts_at" id="starts_at" value="{{ old('starts_at', optional($raffleGame->starts_at)->format('Y-m-d\\TH:i')) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ends_at') ? 'is-invalid' : '' }}" type="datetime-local" name="ends_at" id="ends_at" value="{{ old('ends_at', optional($raffleGame->ends_at)->format('Y-m-d\\TH:i')) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $raffleGame->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('commission_percent') ? 'is-invalid' : '' }}" type="number" name="commission_percent" id="commission_percent" value="{{ old('commission_percent', $raffleGame->commission_percent) }}" min="0" max="100" step="0.01" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ old('active', $raffleGame->active) == 1 ? 'checked' : '' }}>
